...
export const nodePrevSibling = (node: SyntaxNode | null) => {
...
export const nodeRightMostChildBefore = (node: SyntaxNode | null, pos: number) => {
...
export const createComplete =
  ...
  async (ctx: CompletionContext) => {
    ...
    } else if (nodeBefore.name !== 'ValueRParen' && (n = nodeAncestor(nodeBefore, ['Value']))) {
      const c = nodePrevSibling(nodePrevSibling(n));
      if (c?.name === 'Column') {
        let prefix: string | undefined = ctx.state.sliceDoc(nodeBefore.from, nodeBefore.to);
        completion.from = nodeBefore.from;
        completion.to = nodeBefore.to;
        if (['ValueLParen', 'ValueComma', 'ListOfValues'].includes(nodeBefore.name)) {
          // Empty term
          prefix = undefined;
        ...
        await remote(
          ...
          (o) => (nodeAncestor(nodeBefore, ['ListOfValues', 'ValueLParen']) ? { ...o, apply: `${o.label},` } : o)
        );
      }
    ...
    completion.options.forEach((option) => {
      ...
      if (completion.from > 0 && !/[\s(]/.test(ctx.state.sliceDoc(from - 1, from))) {
