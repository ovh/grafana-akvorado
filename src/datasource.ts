...
export class DataSource extends DataSourceApi<MyQuery, MyDataSourceOptions> {
  ...
  const dataPromises = options.targets.map(async (target) => {
    ...
    const body = {
      ...
      limit: this.computeIntValue(target.limit,options),
      points: 200,
      'previous-period': false,
      start: from,
      'truncate-v4': this.computeIntValue(target.truncatev4 ?? '32',options),
      'truncate-v6': this.computeIntValue(target.truncatev6 ?? '128',options),
